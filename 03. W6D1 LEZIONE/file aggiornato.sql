...
-- ESERCIZIO
    ...
    SELECT SalesOrderNumber, SalesOrderLineNumber, OrderDate, ProductKey, OrderQuantity, UnitPrice, TotalProductCost, SalesAmount
    FROM factresellersales;
    
    -- arricchisci la query precedente del calcolo dei Profitti (SalesAmount - TotalProductCost). Rendi il risultato parlante!
	SELECT SalesOrderNumber, SalesOrderLineNumber, OrderDate, ProductKey, OrderQuantity, UnitPrice, TotalProductCost, SalesAmount, SalesAmount - TotalProductCost AS GrossProfit
    FROM factresellersales;
-- FINE ESERCIZIO
...
FROM factresellersales
WHERE SalesAmount - TotalProductCost <= -1;

-- espoini le transazioni di vendita dei prodotti 349, 275 con profitto superiore a 300
SELECT SalesOrderNumber, SalesOrderLineNumber, OrderDate, ProductKey, OrderQuantity, UnitPrice, TotalProductCost, SalesAmount, SalesAmount - TotalProductCost AS Profit
FROM factresellersales
WHERE ProductKey= 349 OR ProductKey = 275 AND SalesAmount - TotalProductCost >300;

SELECT SalesOrderNumber, SalesOrderLineNumber, OrderDate, ProductKey, OrderQuantity, UnitPrice, TotalProductCost, SalesAmount, SalesAmount - TotalProductCost AS Profit
FROM factresellersales
WHERE SalesAmount - TotalProductCost >300 AND ProductKey IN(349,275);
-- FINE ESERCIZIO
